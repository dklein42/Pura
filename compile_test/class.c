...
/* TODO: index 0 of the contant pool shall not be used! Do we have this working? */ 
...
void readStringInfo( Class* cls, ClassLoaderState* cl, int constantPoolPos )
{
	...
	ref->stringRef= NULL_REFERENCE; /* stores according String instance aftr resolution */ 
...
Class* cls_resolveConstantPoolIndexToClass( Class* cls, int index )
{
	...
	/* See if the requested class has already been loaded (i.e. is in the method area). If not, load it now, and return. */
	return ma_getClass( cls_resolveConstantPoolIndexToUtf8(cls, info->name_index) );
}

/* Loads int or float values (32 bit, one slot) from constant pool entries.
	TODO: This method is also supposed to resolve cp entries of type CONSTANT_String to a string object, which should be returned then. */
...
{
	/* use our very own boolean flags here */
	...
	for( i= 0; i < fieldsCount; i++ )
	{
		...
		if( attributes_count > 0 )
		{
			...
			for( j= 0; j < attributes_count; j++ )
			{
				...
				/* TODO: What is the ConstantValue attribute good for? I don't have any use for it yet. */
...
CONSTANT_NameAndType_info* cls_resolveConstantPoolIndexToNameAndType( Class* cls, uint16 index )
{
	...
	return (CONSTANT_NameAndType_info*)cpEntry;
}
...
void cls_resolveConstantPoolIndexToClassAndMethodInfo( Class* cls, uint16 index, Class** otherClass, method_info** methodInfo )
{
	...
	char* className= cls_resolveConstantPoolIndexToClassName( cls, constMethodInfo->class_index );
	
	CONSTANT_NameAndType_info* nameAndType= cls_resolveConstantPoolIndexToNameAndType( cls, constMethodInfo->name_and_type_index );
	char* methodName= cls_resolveConstantPoolIndexToUtf8( cls, nameAndType->name_index );
	char* methodDescriptor= cls_resolveConstantPoolIndexToUtf8( cls, nameAndType->descriptor_index );
	
	/* get appropriate class from method area, load if not present */
	*otherClass= ma_getClass( className );
	
	/* get appropriate method_info */
	*methodInfo= cls_resolveMethod( otherClass, methodName, methodDescriptor );
...
void cls_resolveConstantPoolIndexToClassAndInterfaceMethodInfo( Class* cls, uint16 index, Class** objRefClass, method_info** methodInfo )
{
	...
	char* className= cls_resolveConstantPoolIndexToClassName( cls, constInterfaceMethodInfo->class_index );
	...
	/* Get appropriate interface class from method area, load if not present. */
	Class* interfaceClass= ma_getClass( className );
	
	/* Resolve appropriate method from the given class or its super classes. */
	method_info* interfaceMethodInfo= cls_resolveMethod( &interfaceClass, methodName, methodDescriptor );
	
	/* Now verify that we correctly have an interface class with a matching method declaration here. */
	if( !isFlagSet(interfaceClass->access_flags, ACC_INTERFACE) )
	...
	if( interfaceMethodInfo == NULL )
	{
		...
		char* methodName;
		char* methodDesc;
		cls_resolveConstantPoolIndexOfMethodRefToMethodNameAndDescriptor( interfaceClass, index, &className, &methodName, &methodDesc );
	...
	*methodInfo= cls_resolveMethod( objRefClass, methodName, methodDescriptor );	
	return;
}

/* Resolves a constant pool entry of a methodref into the method's name. */
/* TODO: Optimize resolution! */
...
void cls_resolveConstantPoolIndexToClassAndVariableInfo( Class* cls, uint16 index, Class** otherClass, variable** variableInfo, boolean isStaticField )
{
	...
	char* className= cls_resolveConstantPoolIndexToClassName( cls, fieldrefInfo->class_index );
	
	CONSTANT_NameAndType_info* nameAndType= cls_resolveConstantPoolIndexToNameAndType( cls, fieldrefInfo->name_and_type_index );
	char* fieldName= cls_resolveConstantPoolIndexToUtf8( cls, nameAndType->name_index );
	char* fieldDescriptor= cls_resolveConstantPoolIndexToUtf8( cls, nameAndType->descriptor_index );
	
	/* get appropriate class from method area, load if not present */
	*otherClass= ma_getClass( className );
	
	/* get appropriate field_info (which probably also adjusts the class */
	if( isStaticField )
		*variableInfo= cls_resolveStaticField( otherClass, fieldName, fieldDescriptor );
	else
		*variableInfo= cls_resolveField( otherClass, fieldName, fieldDescriptor );
	return;
}
