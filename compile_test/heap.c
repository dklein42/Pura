...
/* Returns an unused entry in the object pointer list.
	Note, that this method currently is NOT thread safe!
	TODO: Implement a better version, that is also able to find empty spots in the list, if the end is reached. Also increase the list size if required. */
...
/* Initializes the datastructure(s) of the heap. */
void heap_init()
{
...
/* Creates a new instance of an array with a maximum data type size of one byte. */ 
...
/* Recursively checks the given object if it or its super classes are an instance of the given class. */
